...
async function init(agentId: number) {
	...
	const requestedSessionId = String(route.query.sessionId || '');
	if (requestedSessionId) {
		const requestedSession = store.sessions.find(
			(session) => session.id === requestedSessionId,
		);
		if (requestedSession && store.currentSession?.id !== requestedSession.id) {
			await store.selectSession(requestedSession);
		}
	}
}
